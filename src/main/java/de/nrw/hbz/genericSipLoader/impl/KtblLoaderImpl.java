...
public class KtblLoaderImpl {
  ...
  /**
   * Adds metadata as N-Triples to the fedora representation of any to.science
   * Object
   * 
  ...
  /**
   * Method (enhanced from cuToScienceObject) aims to persist a complete ktbl
  ...
  public void addRelatededDataSetMD(LinkedHashMap<String, String> ktblDataId) {
      ...
      if (ktblJSONObj.has("relatedDatasets")) {
        ...
        while (kIt.hasNext()) {
          ...
          if (localFileList != null && localFileList.size() > 0) {
            ...
            Hashtable<String, String> related = new Hashtable<>();
            related.put("prefLabel", prefLabel);
            related.put("id", id);
            relatedDataset.add(related);
            logger.info("KeySet: " + localFileList.get(j) + ", " + j);
            logger.info("id : " + id);
            logger.info("preLabel: " + prefLabel);
    ...
    for (int i = 0; i < localFileList.size(); i++) {
      ...
      logger.info("Anzahl Relationen: " + relatedDataset.size());
      ...
      logger.info(ktblJSONObj.toString(2));
  ...
  /**
   * This method checks if there are other files in the same folder where the json
   * file is located
   * 
